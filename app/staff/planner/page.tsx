...
export default function PlannerPage() {
    ...
    return (
        <div className="flex h-screen w-full bg-toast-charcoal text-white font-sans selection:bg-toast-orange selection:text-white relative">
            ...
            <main className="flex-1 flex flex-col h-full bg-[#1a1a1a] overflow-hidden">
                {/* HEADER */}
                <header className="flex items-center justify-between px-6 py-4 bg-toast-charcoal border-b border-white/5 shadow-md z-20">
                    <div className="flex items-center gap-4">
                        <div className="flex items-center gap-1 text-gray-400 text-xs font-bold uppercase tracking-wider">
                            <span className="text-toast-orange">RRHH</span>
                            <span className="text-gray-600">/</span>
                            <span>PLANIFICADOR</span>
                        </div>
                        <h1 className="text-2xl font-bold text-white tracking-tight flex items-center gap-2">
                            PLANIFICADOR <span className="text-xs px-2 py-0.5 rounded bg-toast-orange text-white">v2.0 SECURE</span> <span className="text-xs px-2 py-0.5 rounded bg-green-900/50 text-green-400 border border-green-500/20 flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse"></span>ONLINE</span>
                        </h1>
                    </div>

                    {/* ... (rest of header content controls) ... */}
                    <div className="flex items-center gap-4">
                        <div className="flex bg-black/30 p-1 rounded-lg border border-white/5">
                            ...
                            <div className="px-4 py-1 text-sm font-bold text-gray-200 flex items-center gap-2 border-l border-r border-white/5">
                                <Calendar className="w-4 h-4 text-toast-orange" />
                                {format(currentWeekStart, 'dd MMM', { locale: es })} - {format(addDays(currentWeekStart, 6), 'dd MMM', { locale: es })}
                            </div>
                            <button
                                onClick={() => setCurrentWeekStart(addDays(currentWeekStart, 7))}
                                className="p-1.5 hover:bg-white/10 rounded-md transition-colors"
                        ...
                        <button
                            ...
                            className="bg-purple-600 hover:bg-purple-500 text-white px-4 py-2 rounded-lg font-bold text-sm tracking-wide shadow-lg shadow-purple-900/20 flex items-center gap-2 transition-all disabled:opacity-50"
                        >
                            <BrainCircuit className={`w-4 h-4 ${isGeneratingAi ? 'animate-spin' : ''}`} />
                            {isGeneratingAi ? 'GENERANDO...' : 'SUGERIR IA'}
                        </button>

                        <button
                            className="bg-toast-orange hover:bg-orange-600 text-white px-4 py-2 rounded-lg font-bold text-sm tracking-wide shadow-lg shadow-orange-900/20 flex items-center gap-2 transition-all"
                        >
                            <Save className="w-4 h-4" />
                            PUBLICAR
                        </button>
                        {/* REST OF HEADER (Sync Status, Clock, User) */}
                        ...
                        <div className="px-4 py-1.5 rounded-lg bg-black/20 border border-white/5 text-gray-400 text-sm font-mono flex items-center gap-2">
                            <span onClick={() => window.location.reload()} className="cursor-pointer hover:text-white flex items-center gap-1"><span className="text-xs">↻</span> Restaurar</span>
                            <span className="text-white font-bold text-base border-l border-white/10 pl-2 ml-1">
                        ...
                        <div className="flex items-center gap-3 pl-2">
                            <div className="text-right hidden md:block">
                                <p className="text-sm font-bold text-white leading-tight">Ricardo Peralta</p>
                                <p className="text-[10px] font-bold text-blue-400 uppercase tracking-wider">GERENTE</p>
                            </div>
                            <div className="w-9 h-9 rounded-full bg-white/10 flex items-center justify-center text-sm font-bold border border-white/10">
                                R
                            </div>
                        </div>
    ...
    const renderPlannerGrid = (start: Date, title: string, isFuture: boolean = false) => {
        const days = Array.from({length: daysToShow }, (_, i) => addDays(start, i));
                    ...
                    return (
                    <div className="min-h-screen bg-[#1e1e1e] text-white font-sans text-sm flex flex-col relative h-screen">
                        {/* Header Toolbar */}
